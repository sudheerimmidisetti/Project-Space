...
router.post("/forgotpassword", async (req, res) => {
  try {
    ...
    const frontendUrl = (
      process.env.FRONTEND_URL || "http://localhost:5173/Project-Space"
    ).replace(/\/$/, "");
    ...
    try {
      ...
      return res
        .status(500)
        .json({
          message: "Email could not be sent. Check backend/.env credentials",
        });
    }
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ message: "Server error during forgot password Request" });
  }
});
